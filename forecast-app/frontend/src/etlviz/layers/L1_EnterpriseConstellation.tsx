...
export default function L1_EnterpriseConstellation() {
  const { tierData, vectorResults, setVectorResults, drillDown, uploadId } = useNavigationContext();
  ...
  useEffect(() => {
    if (tierData && !vectorResults && uploadId) {
      setLoading(true);
      analyzeVectorsStream(tierData, uploadId, (ev: VectorStreamEvent) => {
        if (ev.phase === 'complete' && ev.result) {
          setVectorResults(ev.result);
          setLoading(false);
        } else if (ev.phase === 'error') {
          console.error('Vector analysis failed:', ev.message);
          setLoading(false);
        }
      });
    }
  }, [tierData, vectorResults, setVectorResults, uploadId]);
