...
const FolderDependencyDiagram = lazy(() => import('./FolderDependencyDiagram'));
...
type ViewId = 'tier' | 'constellation' | 'explorer' | 'conflicts' | 'order' | 'matrix'
  | 'tables' | 'duplicates' | 'chunking' | 'folders' | 'folderdeps'
  | 'complexity' | 'heatmap' | 'concentration'
  | 'profile' | 'flowwalker' | 'chat' | 'admin'
  | 'decisiontree' | 'algorithmlab' | 'compare';

// Group determines tab section: core, harmonize, ai, nav
const VIEWS: { id: ViewId; label: string; icon: string; group?: 'core' | 'ai' | 'nav' | 'harmonize' }[] = [
  ...
  { id: 'folderdeps', label: 'Folder Deps', icon: '\uD83D\uDD17', group: 'harmonize' },
  { id: 'complexity', label: 'Complexity', icon: '\u25A3', group: 'ai' },
  { id: 'heatmap', label: 'Heat Map', icon: '\u2593', group: 'ai' },
  { id: 'concentration', label: 'Gravity', icon: '\u2295', group: 'ai' },
  { id: 'algorithmlab', label: 'Algorithm Lab', icon: '\u2697', group: 'ai' },
  { id: 'flowwalker', label: 'Flow', icon: '\u21C4', group: 'ai' },
...
const AI_VIEW_IDS = new Set(VIEWS.filter(v => v.group === 'ai').map(v => v.id));
...
export function DependencyApp() {
  ...
  // ── Inscope filter (global — applied to all views via scopedTierData) ──────
  const [inscopeFilter, setInscopeFilter] = useState<'all' | 'yes' | 'no'>('all');
  const hasInscopeData = useMemo(() => tierData?.sessions?.some(s => s.inscope) ?? false, [tierData]);
  ...
  const scopedTierData = useMemo(() => {
    ...
    // 2. Inscope filter
    if (inscopeFilter !== 'all' && data) {
      const wantInscope = inscopeFilter === 'yes';
      const sessions = data.sessions.filter(s => !!s.inscope === wantInscope);
      const sessionIds = new Set(sessions.map(s => s.id));
      const connections = data.connections.filter(c => sessionIds.has(c.from) || sessionIds.has(c.to));
      const tableIds = new Set<string>();
      connections.forEach(c => {
    ...
    return data;
  }, [tierData, hasChunkSelection, selectedChunks, inscopeFilter, drillMatchingIds]);
            ...
            {VIEWS.filter(v => v.group !== 'ai').map(v => (
              <button
                ...
                style={{
                  ...
                  color: view === v.id ? T.accentText : T.textMuted,
            ...
            <div ref={aiMenuRef} style={{ position: 'relative' }}>
              <button
                ...
                style={{
                  padding: '5px 10px', borderRadius: 6, border: 'none',
                  cursor: 'pointer', fontSize: 11, fontWeight: 600,
                  background: AI_VIEW_IDS.has(view) ? T.accentBg : aiMenuOpen ? 'rgba(99,102,241,0.15)' : 'transparent',
                  color: AI_VIEW_IDS.has(view) ? T.accentText : T.textMuted,
                  transition: 'all 0.15s',
                }}
              >
                &#x2728; AI &#x25BE;
              ...
              {aiMenuOpen && (
                <div style={{
                  ...
                  background: T.bgBar, border: `1px solid ${T.border}`, borderRadius: 8,
                  boxShadow: '0 8px 24px rgba(0,0,0,0.4)', minWidth: 180, overflow: 'hidden',
                }}>
                  {VIEWS.filter(v => v.group === 'ai').map(v => (
                    <button
                      key={v.id}
                      onClick={() => { navigateView(v.id); setAiMenuOpen(false); }}
                      style={{
                      ...
                      onMouseEnter={e => { if (view !== v.id) (e.target as HTMLElement).style.background = 'rgba(99,102,241,0.1)'; }}
                      onMouseLeave={e => { if (view !== v.id) (e.target as HTMLElement).style.background = 'transparent'; }}
                    >
                      {v.icon} {v.label}
                    </button>
                  ))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {hasInscopeData && (
            <div style={{
              ...
              border: `1px solid ${T.border}`, marginRight: 8, fontSize: 10, fontWeight: 600,
            }}>
              <button
                onClick={() => setInscopeFilter(inscopeFilter === 'yes' ? 'all' : 'yes')}
                style={{
                  padding: '4px 10px', border: 'none', cursor: 'pointer',
                  background: inscopeFilter === 'yes' ? '#10B981' : 'transparent',
                  color: inscopeFilter === 'yes' ? '#fff' : T.textMuted,
                  transition: 'all 0.15s',
                }}
              >
                In Scope
              ...
              <button
                onClick={() => setInscopeFilter(inscopeFilter === 'no' ? 'all' : 'no')}
                style={{
                  padding: '4px 10px', border: 'none', cursor: 'pointer',
                  borderLeft: `1px solid ${T.border}`,
                  background: inscopeFilter === 'no' ? '#EF4444' : 'transparent',
                  color: inscopeFilter === 'no' ? '#fff' : T.textMuted,
                  transition: 'all 0.15s',
                }}
              >
                Out of Scope
          ...
          <button
            ...
            style={{
              ...
              color: rightPanel === 'export' ? T.accentText : T.textMuted, fontSize: 13, cursor: 'pointer',
            }}
          >
            &#x2B07;
          </button>
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1, color: '#8899aa' }}>Loading view...</div>}>
            <div style={{ flex: 1, overflow: 'hidden', padding: (['tier', 'matrix', 'constellation', 'tables', 'duplicates', 'heatmap', 'umap', 'decisiontree', 'flowwalker', 'algorithmlab', 'compare', 'folders'].includes(view)) ? 0 : 20 }}>
              ...
              {view === 'folderdeps' && scopedTierData && (
                <ErrorBoundary>
                  <div style={{ overflow: 'hidden', height: '100%' }}>
                    <FolderDependencyDiagram data={scopedTierData} />
                  </div>
                </ErrorBoundary>
              )}
