...
from etlviz.config import settings
from etlviz.models.database import Upload, get_db
...
def _get_analyzer(platform: str):
    ...
    if platform == 'informatica':
        from etlviz.engines.infa_engine import analyze
        return analyze
    else:
        from etlviz.engines.nifi_tier_engine import analyze
        return analyze
...
async def _analyze_mixed(
    ...
    if infa_raw:
        from etlviz.engines.infa_engine import analyze as infa_analyze
        r = await asyncio.to_thread(infa_analyze, infa_raw, infa_names, progress_fn)
        results.append(r)

    # Run NiFi engine if any NiFi files were detected
    if nifi_raw:
        from etlviz.engines.nifi_tier_engine import analyze as nifi_analyze
...
async def _extract_xml_from_path(file_path: str) -> tuple[list[bytes], list[str]]:
    ...
    """
    from etlviz.engines.dbfs_reader import read_file
...
async def analyze_tier_map(
    ...
    try:
        ...
        from etlviz.engines.data_populator import populate_core_tables, populate_view_tables, populate_code_analysis_tables, populate_deep_parse_tables, populate_normalized_tables
        try:
...
async def analyze_constellation(
    ...
    try:
        ...
        from etlviz.engines.constellation_engine import build_constellation
        ...
        # Populate per-view materialized tables
        from etlviz.engines.data_populator import populate_core_tables, populate_view_tables, populate_constellation_tables, populate_code_analysis_tables, populate_deep_parse_tables, populate_normalized_tables
        try:
...
def _validate_server_path(file_path: str) -> None:
    ...
    from etlviz.engines.dbfs_reader import is_dbfs_path
...
async def list_algorithms():
    ...
    """
    from etlviz.engines.constellation_engine import ALGORITHMS
...
async def list_lab_algorithms():
    ...
    from etlviz.engines.algorithm_lab_engine import LAB_ALGORITHMS
...
async def run_lab_algorithm_endpoint(
    ...
    from etlviz.engines.algorithm_lab_engine import run_lab_algorithm
...
def get_upload(upload_id: int, db: Session = Depends(get_db)):
    ...
    if not tier_data or not tier_data.get('sessions'):
        from etlviz.engines.data_populator import reconstruct_tier_data
    ...
    if not constellation or not constellation.get('points'):
        from etlviz.engines.data_populator import reconstruct_constellation
...
def list_sessions(
    ...
    """
    from etlviz.models.database import SessionRecord
...
# ── Inscope tagging endpoint ──────────────────────────────────────────────────

@router.post('/tier-map/uploads/{upload_id}/inscope')
def set_inscope(
    upload_id: int,
    file: UploadFile = File(...),
    db: Session = Depends(get_db),
):
    """Bulk-set the inscope flag on sessions from an uploaded Excel file.

    Reads a 'Session_Name' column from the Excel file and marks matching
    sessions as inscope=1. All other sessions for this upload are set to
    inscope=0.

    Args:
        upload_id: DB primary key of the upload.
        file: Excel (.xlsx) file with a Session_Name column.
        db: SQLAlchemy session (injected).

    Returns:
        Dict with matched/unmatched counts.
    """
    from etlviz.models.database import SessionRecord
    ...
    if 'Session_Name' not in headers:
        raise HTTPException(status_code=400, detail='Excel file must have a Session_Name column.')
    col_idx = headers.index('Session_Name')

    inscope_names: set[str] = set()
    for row_data in ws.iter_rows(min_row=2, values_only=True):
        val = row_data[col_idx]
        if val:
            inscope_names.add(str(val).strip())
    wb.close()

    # Reset all sessions to out-of-scope first
    db.query(SessionRecord).filter(
        SessionRecord.upload_id == upload_id,
    ).update({SessionRecord.inscope: 0})

    # Mark matching sessions as in-scope
    ...
    matched = 0
    for s in sessions:
        if s.name in inscope_names:
            s.inscope = 1
            matched += 1

    db.commit()

    # Invalidate tier_data cache so reconstruct picks up inscope
    from etlviz.engines.data_populator import invalidate_tier_cache
    try:
        invalidate_tier_cache(upload_id)
    except Exception:
        pass

    return {
        'matched': matched,
        'total_inscope_names': len(inscope_names),
        'unmatched': len(inscope_names) - matched,
