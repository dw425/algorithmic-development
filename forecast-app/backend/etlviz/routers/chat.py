...
from etlviz.engines.data_populator import reconstruct_tier_data, reconstruct_vector_results
...
def _get_engines() -> dict:
    ...
    with _engines_lock:
        ...
        try:
            from etlviz.engines.embedding_engine import EmbeddingEngine
            from etlviz.engines.vector_store import VectorStore
            from etlviz.engines.query_engine import HybridSearchEngine, RAGChatEngine
        ...
        if settings.databricks_app:
            from etlviz.engines.pg_vector_store import PgVectorStore
            store = PgVectorStore()
...
def _run_indexing_background(upload_id: int, tier_data: dict, vector_results: dict | None):
    ...
    try:
        from etlviz.engines.indexing_pipeline import IndexingPipeline
