...
def populate_core_tables(
    ...
    for s in tier_data.get('sessions', []):
        ...
        session_records.append(SessionRecord(
            ...
            inscope=1 if s.get('inscope') else 0,
        ))
...
def reconstruct_tier_data(db: Session, upload_id: int) -> dict | None:
    ...
    for s in sessions:
        session_list.append({
            ...
            'mapping': s.mapping_name,
            'config_reference': s.config_reference,
            'total_loc': s.total_loc,
            'has_embedded_sql': s.has_embedded_sql,
            'core_intent': s.core_intent,
            'inscope': bool(s.inscope),
        })
