...
from etlviz.models.database import init_db
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    """
    from etlviz.utils.errors import ETLMigrationError
    cid = correlation_id.get('-')
...
async def health():
    ...
    try:
        ...
        from etlviz.models.database import SessionLocal, engine
    ...
    try:
        from etlviz.models.database import engine
        pool = engine.pool
    ...
    try:
        from etlviz.models.database import SessionLocal, DocumentEmbedding
        db = SessionLocal()
...
async def health_metrics():
    ...
    try:
        from etlviz.engines.databricks_llm import DatabricksLLM
        snapshot["llm_usage"] = DatabricksLLM.get_usage_stats()
    except Exception:
        pass
    try:
        from etlviz.engines.databricks_embeddings import DatabricksEmbeddingEngine
...
async def health_slow_queries(limit: int = Query(20, ge=1, le=100)):
    ...
    from etlviz.models.database import _slow_query_log
...
from etlviz.routers.tier_map import router as tier_map_router
from etlviz.routers.vectors import router as vectors_router
from etlviz.routers.layers import router as layers_router
from etlviz.routers.active_tags import router as active_tags_router
from etlviz.routers.users import router as users_router
from etlviz.routers.lineage import router as lineage_router
from etlviz.routers.exports import router as exports_router
from etlviz.routers.chat import router as chat_router
from etlviz.routers.views import router as views_router
from etlviz.routers.projects import router as projects_router
from etlviz.routers.compare import router as compare_router
...
async def admin_update_upload(upload_id: int, body: dict):
    ...
    from etlviz.models.database import SessionLocal, Upload
