...
class SessionRecord(Base):
    ...
    # Inscope flag — 1 = in scope, 0 = out of scope
    inscope = Column(Integer, default=0)
...
def _migrate_db() -> None:
    ...
    if "session_records" in insp.get_table_names():
        ...
        # Inscope tagging column
        if "inscope" not in sr_cols:
            with engine.begin() as conn:
                conn.execute(sa_text("ALTER TABLE session_records ADD COLUMN inscope INTEGER DEFAULT 0"))
                logger.info("Migrated: added inscope column to session_records")
